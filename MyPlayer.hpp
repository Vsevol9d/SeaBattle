...
struct MyPlayer:  public IPlayer {
    ...
    bool create_ship(int x1, int y1, int x2, int y2){
     ...
     pair<int, int> shot() {
        int x = last_shot.first, y = last_shot.second;
        if (check(x, y) && enemy[x][y] == 'P') {
            if (check(x + 1, y) && check(x + 1, y) && enemy[x + 1][y] == 'P') last_shot = {x - 1, y};
            else if (check(x - 1, y) && check(x + 1, y) && enemy[x - 1][y] == 'P') last_shot = {x + 1, y};
            else if (check(x, y + 1) && check(x, y - 1) && enemy[x][y + 1] == 'P') last_shot = {x, y - 1};
            else if (check(x, y - 1) && check(x, y + 1) && enemy[x][y - 1] == 'P') last_shot = {x, y + 1};
            else {
                last_shot = {rand() % 10, rand() % 10};
                while (enemy[last_shot.first][last_shot.second] == '*' || enemy[last_shot.first][last_shot.second] == 'K' || enemy[last_shot.first][last_shot.second] == 'P') {
                    last_shot = {rand() % 10, rand() % 10};
                }
            }
        }
